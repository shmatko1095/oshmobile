...
- The dashboard renders the room AppBar, temperature sensor carousel,
  `dailyStats24h`, and the fixed mode bar.
- When a renderable `heatingToggle` is present, the schema promotes its bind to
  `ThermostatDashboardSchema.heatingStatus`. The selected temperature card uses
  that bind for the device-level flame indicator while the original status tile
  remains in the live-values surface. Missing and unreadable configurations do
  not produce the promoted status.
- In portrait, the temperature carousel viewport is full-bleed. The selected
  ...
  discoverability. `dailyStats24h` and the mode bar keep their 20 dp horizontal
  inset. Landscape keeps the inset around the side-by-side carousel and stats
  layout.
...
- Existing tile widgets are reused inside `ThermostatLiveMetricsSheet`, so their
  live MQTT bindings and history navigation remain compatible with legacy
  configurations.
...
for carousel, stats, mode-bar, hint, and compact/landscape allocation; widget
