...
### 3. Подпись Android должна настраиваться вне репозитория (`android/key.properties`)
Файл конфигурации подписи: [android/app/build.gradle](android/app/build.gradle#L12)

`android/key.properties` и upload keystore не должны храниться в Git (и обычно добавлены в `.gitignore`), но **обязаны** быть настроены на билд-машине/в CI. Если их нет в окружении сборки, release AAB будет неподписанным/непригодным для публикации в Google Play.
...
- Логирование централизовано через `package:logging` + bootstrap-пайплайн; в release в консоль идут только `error` (verbose можно временно включить через `--dart-define=ENABLE_VERBOSE_LOGS=true`).
...
- Secure storage, JWT decode, BLE provisioning, QR-сканер, permission_handler — подключены и имеют корректные usage description.
...
- iOS usage descriptions: Photo library, Camera, Bluetooth Always / Peripheral, Location When In Use
...
3. [ ] Устранить `TODO_SECURE_CODE` в `device_offline_page.dart` и TODO в `auth_repository_impl.dart`.
4. [ ] Принять решение по `largeScreens` / `xlargeScreens` в AndroidManifest.
5. [ ] Обновить `README.md` и подготовить release notes для `1.0.7`.
6. [ ] Проверить отсутствие `lib/device_mock/main_mock.dart` в release entrypoint.
7. [ ] Прогнать `flutter build appbundle --release` и `flutter build ipa --release` на чистой машине, проверить размер, подпись, запуск на реальном устройстве.
8. [ ] Проверить mobile client policy (`/v1/mobile/client-policy`) на prod-окружении для текущей версии.

## Итог

Код и тесты в хорошей форме, функционал v1 готов. В **текущем** виде выкладывать в Google Play / App Store нельзя — минимально необходимо: переключить auth на prod и убедиться в корректной подписи Android-билда в CI. После закрытия блокеров проект готов к релизу.
